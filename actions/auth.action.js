...
const actions = {
	login: {
		...
		async handler (ctx) {
			...
			const users = await ctx.call("users.find",
				{ populate: ["+password"], query: { $or: [{ email: user_name }, { phone: user_name }] } });
			if (!users.length) {
			...
			return responder.httpOK(token, authTransformer);
		},
	},
	...
	me: {
		...
		async handler (ctx) {
			const users = await ctx.call("users.find", { query: {_id: ctx.meta.auth.id}});
			if (!users.length) {
				return responder.httpBadRequest(translate("unauthorized"), { user_name: translate("user_name_invalid") });
			}
			const user = { ...users[0] };
